...
async function generateIntervention(event) {
  ...
  try {
    const data = await callApi('/intervention', { state, mood, blocker, tone });
    const message = `
      <p><strong>${data.opener}</strong></p>
      <p>${data.tone}</p>
      <p>${data.mood_hint}</p>
      <p>${data.blocker_hint}</p>
      <p class="highlight">试试这个 life hack：${data.hack}</p>
    `;
    ...
    const message = `
      ...
      <p class="muted">（后台未响应，已使用离线规则生成）</p>
    `;
    document.getElementById('interventionOutput').innerHTML = message;
  }
}
...
async function generateReflection(event) {
  ...
  try {
    const data = await callApi('/reflection', {
      dayQuality: quality,
      highlight,
      block,
      experiment,
    ...
    const summary = `
      ...
      <p class="highlight">明日提示：${data.next_step}</p>
    `;
    ...
    document.getElementById('reflectionOutput').innerHTML = fallback;
  }
}
